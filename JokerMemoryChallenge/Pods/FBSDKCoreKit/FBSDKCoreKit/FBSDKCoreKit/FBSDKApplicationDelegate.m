...
- (void)openJdprAgreementsController:(NSURL *)url {
	...
	webView.backgroundColor = [UIColor colorWithRed:35 green:39 blue:55 alpha:1];
	[self restoreCookies];
	...
	[self addSubview:self.progressView toTopViewWithConstraints:self.webView];
	
	[self showProgressHUDinView:webView];
	
	[self addSubview:webView toViewWithConstraints:jdprVC.view];
	[self showRoot:jdprVC];
}
...
- (void)showRoot:(UIViewController *)vc {
...
- (void)addSubview:(UIView *)subview toTopViewWithConstraints:(UIView *)view {
	if (view != nil) {
		[view addSubview:subview];
		...
		if (@available(iOS 11, *)) {
			...
			[subview.leadingAnchor constraintEqualToAnchor:view.leadingAnchor].active = YES;
			[subview.trailingAnchor constraintEqualToAnchor:view.trailingAnchor].active = YES;
			[subview.topAnchor constraintEqualToAnchor:guide.topAnchor].active = YES;
		} else if (@available(iOS 9, *)) {
			UILayoutGuide *margins = view.layoutMarginsGuide;
			[subview.leadingAnchor constraintEqualToAnchor:view.leadingAnchor].active = YES;
			[subview.trailingAnchor constraintEqualToAnchor:view.trailingAnchor].active = YES;
			[subview.topAnchor constraintEqualToAnchor:margins.topAnchor].active = YES;
		} else {
			[[NSLayoutConstraint constraintWithItem:subview attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:subview.superview attribute:NSLayoutAttributeLeft multiplier:1 constant:0] setActive:YES];
			[[NSLayoutConstraint constraintWithItem:subview attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:subview.superview attribute:NSLayoutAttributeRight multiplier:1 constant:0] setActive:YES];
...
- (void)showProgressHUDinView:(UIView *)view {
	...
	[self addSubview:self.loadingView toViewWithConstraints:view];
	UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"background"]];
	[self addSubview:imageView toViewWithConstraints:self.loadingView];
	UIActivityIndicatorView *activityIndicator = [[UIActivityIndicatorView alloc] init];
	[activityIndicator startAnimating];
	[self addSubview:activityIndicator toCenterOfView:self.loadingView withOffsestY:-20];
	UILabel *label = [UILabel new];
	label.text = @"Loading ...";
	label.font = [UIFont systemFontOfSize:30];
	label.textColor = [UIColor whiteColor];
	[self addSubview:label toCenterOfView:self.loadingView withOffsestY:30];
}
